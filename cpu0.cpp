...
void CPU::execute_r_0_instruction(uint8_t function, uint8_t reg_0, uint8_t reg_1) {
	if (reg_0 == 0 && (function == 0 || function == 2) ) {
		return;
	}
	switch (function) {
		case 0:
		...
		case 1:
			...
			_acc = reg_1 > reg_0 ? 0x00000001 : (reg_1 < reg_0 ? 0xffffffff : 0x00000000);
			break;
		case 2:
			// load $reg_0, [$reg_1]
		...
		case 3:
			// store $reg_0, [$reg_1]
			uint32_t value   = _register_file[reg_0];
			uint32_t address = _register_file[reg_1];
			_memory_file[address    ] = (uint8_t) (value >> 24);
			_memory_file[address + 1] = (uint8_t) (value >> 16);
			_memory_file[address + 2] = (uint8_t) (value >>  8);
			_memory_file[address + 3] = (uint8_t) (value      );
			break;
		case 4:
			// add $reg_0, $reg_1
